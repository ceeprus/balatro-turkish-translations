...
function Game:start_up()
    ...
    local settings = get_compressed_savegame('common', 'settings.jkr')
    ...
    if settings then 
        ...
        if G.VERSION >= '1.0.0' and (love.system.getOS() == 'NOPE') and ((not settings_file.version) or (settings_file.version < '1.0.0')) then 
            for i = 1, 3 do
                love.filesystem.remove("s_"..i..'_'..'profile.jkr')
                love.filesystem.remove("s_"..i..'_'..'save.jkr')
                love.filesystem.remove("s_"..i..'_'..'meta.jkr')
                love.filesystem.remove("s_"..i..'_'..'unlock_notify.jkr')
    ...
    for k, filename in ipairs(shader_files) do
        ...
        if extension == '.fs' then
            ...
            fileData, fileSize = love.filesystem.read("resources/shaders/"..filename)
            self.SHADERS[shader_name] = love.graphics.newShader(fileData)
        end
    end
...
function Game:init_item_prototypes()
    ...
    --if not love.filesystem.exists("s_"..G.SETTINGS.profile..'_'..'meta.jkr') then love.filesystem.append( "s_"..G.SETTINGS.profile..'_'..'meta.jkr', 'return {}') end

    local meta = STR_UNPACK(get_compressed_savegame(G.SETTINGS.profile,'meta.jkr') or 'return {}')
...
function Game:load_profile(_profile)
    ...
    --Load the settings file
    local info = get_compressed_savegame(_profile,'profile.jkr')
    if info ~= nil then
...
function Game:set_language()
    if not self.LANGUAGES then 
        ...
        self.LANGUAGES = {
            ...
            ['tr'] = {font = 1, label = "Türkçe", key = 'tr', beta = nil, button = "Dil Geri Bildirimi", warning = {'Bu dil henüz Beta sürümündedir. Geri bildirim tuşuna tıklayarak','bize yardımcı olabilir ve çeviriyi geliştirebilirsiniz.', 'Doğrulamak için tekrar tıkla'}},
...
function Game:set_render_settings()
    ...
    for i=1, #self.animation_atli do
        ...
        self.ANIMATION_ATLAS[self.animation_atli[i].name].image = love.graphics.newImage(self.animation_atli[i].path, {mipmaps = false, dpiscale = self.SETTINGS.GRAPHICS.texture_scaling})
    ...
    for i=1, #self.asset_atli do
        ...
        self.ASSET_ATLAS[self.asset_atli[i].name].image = love.graphics.newImage(self.asset_atli[i].path, {mipmaps = false, dpiscale = self.SETTINGS.GRAPHICS.texture_scaling})
    ...
    for i=1, #self.asset_images do
        ...
        self.ASSET_ATLAS[self.asset_images[i].name].image = love.graphics.newImage(self.asset_images[i].path, {mipmaps = false, dpiscale = 1})
...
function Game:init_window(reset)
	...
	if love.system.getOS() == 'gdkpc' then
    	G.SETTINGS.QUEUED_CHANGE.screenmode = 'Borderless'
	else
